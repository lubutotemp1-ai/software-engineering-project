...
export default function DoctorSidebar({ activePage, setActivePage, user, onLogout }) {
  ...
  const initials = user?.name ? user.name.split(' ').map(n => n[0]).join('').slice(0,2).toUpperCase() : 'D';
  ...
  return (
    <>
      ...
      >
        ...
        {isExpanded && <span style={{ fontSize: 12, color: '#9CA3AF', paddingLeft: '16px' }}>Doctor Dashboard</span>}
