...
router.get("/doctors", async (req, res) => {
  try {
    const doctors = await db.all_(
      "SELECT id, name, email, phone, department, specialization, created_at FROM doctors ORDER BY name ASC",
    );
    res.json(doctors);
  } catch (err) {
...
router.post("/doctors", async (req, res) => {
  ...
  try {
    const existing = await db.get_("SELECT id FROM doctors WHERE email = ?", [
      email,
    ]);
    if (existing)
      return res
    ...
    const doctor = await db.get_(
      "SELECT id, name, email, phone, department, specialization, created_at FROM doctors WHERE id = ?",
...
router.put("/doctors/:id", async (req, res) => {
  ...
  try {
    const doc = await db.get_("SELECT * FROM doctors WHERE id = ?", [
      req.params.id,
    ]);
...
router.post("/doctors/:id/reset-password", async (req, res) => {
  ...
  try {
    ...
    await db.run_("UPDATE doctors SET password = ? WHERE id = ?", [
      hashedPassword,
      req.params.id,
    ]);
    res.json({
      message: `Password reset for Dr. ${doc.name}. They can now login with the new password.`,
    });
  } catch (err) {
    res.status(500).json({ error: "Server error." });
  }
...
router.get("/patients", async (req, res) => {
  try {
    const patients = await db.all_(
      'SELECT id, name, email, phone, date_of_birth, blood_type, created_at FROM users WHERE role = "patient" ORDER BY name ASC',
    );
    res.json(patients);
  } catch (err) {
    res.status(500).json({ error: "Server error." });
...
router.get("/patients/:id", async (req, res) => {
  try {
    const patient = await db.get_(
      'SELECT id, name, email, phone, date_of_birth, blood_type, created_at FROM users WHERE id = ? AND role = "patient"',
      [req.params.id],
    );
...
router.put("/appointments/:id", async (req, res) => {
  ...
  try {
    const appt = await db.get_("SELECT * FROM appointments WHERE id = ?", [
      req.params.id,
    ]);
